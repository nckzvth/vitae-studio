  ...
  return (
    <Shell className={`inline-editor-shell ${wrapperClassName}`.trim()}>
      {createElement(as, {
        ...
        "aria-label": editable
          ? `Edit ${label} directly in the document`
          : undefined,
        "data-placeholder": editable ? placeholder : undefined,
        "data-empty": editable && value.length === 0 ? true : undefined,
        onFocus: () => {
          if (!editable) return;
  ...
  return (
    <div className="bullet-editor-shell">
      <ul
        ...
        role={editable ? "textbox" : undefined}
        aria-label={
          editable ? "Edit bullet list directly in the document" : undefined
        }
        data-empty={editable && bullets.length === 0 ? true : undefined}
        onFocus={() => {
          if (!editable) return;
