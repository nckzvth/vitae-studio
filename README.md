...
- Direct PDF download from the exact rendered CV pages, with searchable text and links
...
The client-side React application converts every CSV import into a versioned, layout-independent `Project` model. The editor, measured document preview, IndexedDB persistence, JSON backup, and direct PDF output all consume the same rendered pages. PDF export captures each finished paper surface inside the app, adds a searchable text and link layer, and downloads the file without invoking browser printing. Theme and layout tokens stay separate from content so a design can change without data loss.
